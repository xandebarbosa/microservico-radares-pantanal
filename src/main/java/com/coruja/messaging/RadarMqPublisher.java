...
public class RadarMqPublisher {
    ...
    public void publicar(Radars radar) {
        ...
        try {
            ...
            log.info("Mensagem enviada para ao RabbitMQ: {}", mensagem);
        } catch (Exception e) {
    ...
    private String formatarMensagem(Radars radar) {
        return String.format("Pantanal|%s|%s|%s|%s|%s",
                radar.getData(), radar.getHora(), radar.getPlaca(), radar.getPlaca(), radar.getSentido()
        );
    }
